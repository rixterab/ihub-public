# Jira -> Yousign -> Jira Signature

This template installs two iHub flows that send a Jira agreement issue to Yousign for signature, then update the same Jira issue from Yousign webhook status changes.

- `Yousign Outgoing Signature`: Jira -> Yousign, triggered when a Jira issue moves to `Send for Signature`.
- `Yousign Incoming Status`: Yousign -> Jira, triggered by Yousign webhook events.
...
- On completion, downloads the signed PDF from the configured Yousign download path and attaches it to Jira.
- Adds the comment `Agreement signed by customer`.
- Transitions the Jira issue to `Completed`.
- Stores an issue property named `ihub-yousign-<signatureRequestId>-completed` so webhook retries do not duplicate the PDF, comment, or transition.
...
| `customfield_10203` / `cf[10203]` | Yousign signature request id | Single-line text |
...
| `YOUSIGN_BASE_URL` | Both | Yousign API host for the target account or environment. |
| `YOUSIGN_CREATE_SIGNATURE_PATH` | Outgoing | API path that creates and sends a signature request from a template. |
| `YOUSIGN_TEMPLATE_ID` | Outgoing | Yousign template ID used for the agreement. |
| `YOUSIGN_SIGNER_ROLE_NAME` | Outgoing | Signer role name used by the template, if supported by Yousign. |
| `YOUSIGN_WEBHOOK_URL` | Outgoing | Custom webhook trigger URL from the incoming flow. |
| `YOUSIGN_DOWNLOAD_SIGNED_PDF_PATH_PREFIX` | Incoming | API path prefix before the signature request id for downloading the signed PDF. |
| `YOUSIGN_DOWNLOAD_SIGNED_PDF_PATH_SUFFIX` | Incoming | API path suffix after the signature request id for downloading the signed PDF. |
| `COMPLETED_TRANSITION_ID` | Incoming | Jira workflow transition ID that moves the issue to `Completed`. |

## Yousign API Configuration

The template uses a custom `Authorization` header credential named `Yousign Authorization Header`. After import, paste the full header value required by your Yousign account, for example a `Bearer ...`, `API-Key ...`, or other provider-specific authorization value.

Before enabling the flows, verify these provider-specific details against your Yousign tenant and API version:
...
1. Create the `Yousign Incoming Status` flow first and copy its custom webhook trigger URL.
2. Paste that URL into the outgoing flow variable `YOUSIGN_WEBHOOK_URL`.
3. Set the Jira project key, Yousign API host, template ID, create path, and download path variables.
4. Replace all Jira custom field placeholders with the field IDs from the target Jira site.
5. Adjust the create request body and response id JSONPath if the Yousign endpoint requires a different shape.
