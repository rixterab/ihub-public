# Jira -> Signicat -> Jira Signature

This template installs two iHub flows that send a Jira agreement issue to Signicat for signature, then update the same Jira issue from Signicat webhook status changes.

- `Signicat Outgoing Signature`: Jira -> Signicat, triggered when a Jira issue moves to `Send for Signature`.
- `Signicat Incoming Status`: Signicat -> Jira, triggered by Signicat webhook events.
...
- On completion, downloads the signed PDF from the configured Signicat download path and attaches it to Jira.
- Adds the comment `Agreement signed by customer`.
- Transitions the Jira issue to `Completed`.
- Stores an issue property named `ihub-signicat-<signatureRequestId>-completed` so webhook retries do not duplicate the PDF, comment, or transition.
...
| `customfield_10203` / `cf[10203]` | Signicat signature request id | Single-line text |
...
| `SIGNICAT_BASE_URL` | Both | Signicat API host for the target account or environment. |
| `SIGNICAT_CREATE_SIGNATURE_PATH` | Outgoing | API path that creates and sends a signature request from a template. |
| `SIGNICAT_TEMPLATE_ID` | Outgoing | Signicat template ID used for the agreement. |
| `SIGNICAT_SIGNER_ROLE_NAME` | Outgoing | Signer role name used by the template, if supported by Signicat. |
| `SIGNICAT_WEBHOOK_URL` | Outgoing | Custom webhook trigger URL from the incoming flow. |
| `SIGNICAT_DOWNLOAD_SIGNED_PDF_PATH_PREFIX` | Incoming | API path prefix before the signature request id for downloading the signed PDF. |
| `SIGNICAT_DOWNLOAD_SIGNED_PDF_PATH_SUFFIX` | Incoming | API path suffix after the signature request id for downloading the signed PDF. |
| `COMPLETED_TRANSITION_ID` | Incoming | Jira workflow transition ID that moves the issue to `Completed`. |

## Signicat API Configuration

The template uses a custom `Authorization` header credential named `Signicat Authorization Header`. After import, paste the full header value required by your Signicat account, for example a `Bearer ...`, `API-Key ...`, or other provider-specific authorization value.

Before enabling the flows, verify these provider-specific details against your Signicat tenant and API version:
...
1. Create the `Signicat Incoming Status` flow first and copy its custom webhook trigger URL.
2. Paste that URL into the outgoing flow variable `SIGNICAT_WEBHOOK_URL`.
3. Set the Jira project key, Signicat API host, template ID, create path, and download path variables.
4. Replace all Jira custom field placeholders with the field IDs from the target Jira site.
5. Adjust the create request body and response id JSONPath if the Signicat endpoint requires a different shape.
