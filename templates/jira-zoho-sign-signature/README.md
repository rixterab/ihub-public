# Jira -> Zoho Sign -> Jira Signature

This template installs two iHub flows that send a Jira agreement issue to Zoho Sign for signature, then update the same Jira issue from Zoho Sign webhook status changes.

- `Zoho Sign Outgoing Signature`: Jira -> Zoho Sign, triggered when a Jira issue moves to `Send for Signature`.
- `Zoho Sign Incoming Status`: Zoho Sign -> Jira, triggered by Zoho Sign webhook events.
...
- On completion, downloads the signed PDF from the configured Zoho Sign download path and attaches it to Jira.
- Adds the comment `Agreement signed by customer`.
- Transitions the Jira issue to `Completed`.
- Stores an issue property named `ihub-zoho-sign-<signatureRequestId>-completed` so webhook retries do not duplicate the PDF, comment, or transition.
...
| `customfield_10203` / `cf[10203]` | Zoho Sign signature request id | Single-line text |
...
| `ZOHO_SIGN_BASE_URL` | Both | Zoho Sign API host for the target account or environment. |
| `ZOHO_SIGN_CREATE_SIGNATURE_PATH` | Outgoing | API path that creates and sends a signature request from a template. |
| `ZOHO_SIGN_TEMPLATE_ID` | Outgoing | Zoho Sign template ID used for the agreement. |
| `ZOHO_SIGN_SIGNER_ROLE_NAME` | Outgoing | Signer role name used by the template, if supported by Zoho Sign. |
| `ZOHO_SIGN_WEBHOOK_URL` | Outgoing | Custom webhook trigger URL from the incoming flow. |
| `ZOHO_SIGN_DOWNLOAD_SIGNED_PDF_PATH_PREFIX` | Incoming | API path prefix before the signature request id for downloading the signed PDF. |
| `ZOHO_SIGN_DOWNLOAD_SIGNED_PDF_PATH_SUFFIX` | Incoming | API path suffix after the signature request id for downloading the signed PDF. |
...
The template uses a custom `Authorization` header credential named `Zoho Sign Authorization Header`. After import, paste the full header value required by your Zoho Sign account, for example a `Bearer ...`, `API-Key ...`, or other provider-specific authorization value.

Before enabling the flows, verify these provider-specific details against your Zoho Sign tenant and API version:
...
1. Create the `Zoho Sign Incoming Status` flow first and copy its custom webhook trigger URL.
2. Paste that URL into the outgoing flow variable `ZOHO_SIGN_WEBHOOK_URL`.
3. Set the Jira project key, Zoho Sign API host, template ID, create path, and download path variables.
4. Replace all Jira custom field placeholders with the field IDs from the target Jira site.
5. Adjust the create request body and response id JSONPath if the Zoho Sign endpoint requires a different shape.
