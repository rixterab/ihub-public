# Jira -> SignNow -> Jira Signature

This template installs two iHub flows that send a Jira agreement issue to SignNow for signature, then update the same Jira issue from SignNow webhook status changes.

- `SignNow Outgoing Signature`: Jira -> SignNow, triggered when a Jira issue moves to `Send for Signature`.
- `SignNow Incoming Status`: SignNow -> Jira, triggered by SignNow webhook events.
...
- On completion, downloads the signed PDF from the configured SignNow download path and attaches it to Jira.
- Adds the comment `Agreement signed by customer`.
- Transitions the Jira issue to `Completed`.
- Stores an issue property named `ihub-signnow-<signatureRequestId>-completed` so webhook retries do not duplicate the PDF, comment, or transition.
...
| `customfield_10203` / `cf[10203]` | SignNow signature request id | Single-line text |
...
| `SIGNNOW_BASE_URL` | Both | SignNow API host for the target account or environment. |
| `SIGNNOW_CREATE_SIGNATURE_PATH` | Outgoing | API path that creates and sends a signature request from a template. |
| `SIGNNOW_TEMPLATE_ID` | Outgoing | SignNow template ID used for the agreement. |
| `SIGNNOW_SIGNER_ROLE_NAME` | Outgoing | Signer role name used by the template, if supported by SignNow. |
| `SIGNNOW_WEBHOOK_URL` | Outgoing | Custom webhook trigger URL from the incoming flow. |
| `SIGNNOW_DOWNLOAD_SIGNED_PDF_PATH_PREFIX` | Incoming | API path prefix before the signature request id for downloading the signed PDF. |
| `SIGNNOW_DOWNLOAD_SIGNED_PDF_PATH_SUFFIX` | Incoming | API path suffix after the signature request id for downloading the signed PDF. |
| `COMPLETED_TRANSITION_ID` | Incoming | Jira workflow transition ID that moves the issue to `Completed`. |

## SignNow API Configuration

The template uses a custom `Authorization` header credential named `SignNow Authorization Header`. After import, paste the full header value required by your SignNow account, for example a `Bearer ...`, `API-Key ...`, or other provider-specific authorization value.

Before enabling the flows, verify these provider-specific details against your SignNow tenant and API version:
...
1. Create the `SignNow Incoming Status` flow first and copy its custom webhook trigger URL.
2. Paste that URL into the outgoing flow variable `SIGNNOW_WEBHOOK_URL`.
3. Set the Jira project key, SignNow API host, template ID, create path, and download path variables.
4. Replace all Jira custom field placeholders with the field IDs from the target Jira site.
5. Adjust the create request body and response id JSONPath if the SignNow endpoint requires a different shape.
